...
void main() {
  vec2 fp = fract(gl_TexCoord[0].xy * color_texture_sz);

  vec2 dx = gl_TexCoord[1].xy;
  vec2 dy = gl_TexCoord[1].zw;

  vec3 A = texture2D(mpass_texture, gl_TexCoord[0].xy -dx -dy).xyz; 
  vec3 B = texture2D(mpass_texture, gl_TexCoord[0].xy     -dy).xyz; 
  vec3 C = texture2D(mpass_texture, gl_TexCoord[0].xy +dx -dy).xyz; 
  vec3 D = texture2D(mpass_texture, gl_TexCoord[0].xy -dx    ).xyz; 
  vec3 E = texture2D(mpass_texture, gl_TexCoord[0].xy        ).xyz; 
  vec3 F = texture2D(mpass_texture, gl_TexCoord[0].xy +dx    ).xyz; 
  vec3 G = texture2D(mpass_texture, gl_TexCoord[0].xy -dx +dy).xyz; 
  vec3 H = texture2D(mpass_texture, gl_TexCoord[0].xy     +dy).xyz; 
  vec3 I = texture2D(mpass_texture, gl_TexCoord[0].xy +dx +dy).xyz; 

  vec3 A1 = texture2D(mpass_texture, gl_TexCoord[0].xy     -dx -2.0*dy).xyz; 
  vec3 C1 = texture2D(mpass_texture, gl_TexCoord[0].xy     +dx -2.0*dy).xyz;
  vec3 A0 = texture2D(mpass_texture, gl_TexCoord[0].xy -2.0*dx     -dy).xyz;
  vec3 G0 = texture2D(mpass_texture, gl_TexCoord[0].xy -2.0*dx     +dy).xyz;
  vec3 C4 = texture2D(mpass_texture, gl_TexCoord[0].xy +2.0*dx     -dy).xyz;
  vec3 I4 = texture2D(mpass_texture, gl_TexCoord[0].xy +2.0*dx     +dy).xyz;
  vec3 G5 = texture2D(mpass_texture, gl_TexCoord[0].xy     -dx +2.0*dy).xyz;
  vec3 I5 = texture2D(mpass_texture, gl_TexCoord[0].xy     +dx +2.0*dy).xyz;
  vec3 B1 = texture2D(mpass_texture, gl_TexCoord[0].xy         -2.0*dy).xyz;
  vec3 D0 = texture2D(mpass_texture, gl_TexCoord[0].xy -2.0*dx        ).xyz;
  vec3 H5 = texture2D(mpass_texture, gl_TexCoord[0].xy         +2.0*dy).xyz;
  vec3 F4 = texture2D(mpass_texture, gl_TexCoord[0].xy +2.0*dx        ).xyz;
